...
- **记录进度** —— 右键任务或点详情面板里的按钮把它标为已完成；标记意味着它的**全部前置**也完成了，节点会画淡。反过来取消一个任务（哪怕是"因为后面那个已完成"才算完成的中间环节），它和它**后面**的所有任务都会重新亮起来 —— 不可能出现"前置没做完、后续却已完成"。有进度之后，打开依赖视图会在横幅下面列出**这条链上现在就能接的下一步**；从来没标记过就不显示这块。
